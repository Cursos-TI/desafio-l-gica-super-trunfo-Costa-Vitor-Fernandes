...
void calcular_super_poder(struct Carta *carta) {
  ...
  carta->super_poder = (float)carta->populacao + carta->area + carta->pib +
                       (float)carta->n_pontos_turisticos +
...
void CompararTodosAtributos(struct Carta carta1, struct Carta carta2) {
  ...
  printf("Pontos Turisticos: %s venceu (%d)\n",
         (carta1.n_pontos_turisticos > carta2.n_pontos_turisticos) ? "Carta 1"
                                                                   : "Carta 2",
         carta1.n_pontos_turisticos > carta2.n_pontos_turisticos);
  ...
  printf("PIB per Capita: %s venceu (%d)\n",
         (carta1.pib_per_capita > carta2.pib_per_capita) ? "Carta 1"
                                                         : "Carta 2",
...
void CompararUmAtributo(struct Carta carta1, struct Carta carta2,
  ...
  if (atributo_escolhido == 4) {
    ...
    printf("Carta 1 - %s (%s): %.2f hab/km2\n", carta1.nome_cidade,
           carta1.estado, carta1.densidade_populacional);

    printf("Carta 2 - %s (%s): %.2f hab/km2\n", carta2.nome_cidade,
           carta2.estado, carta2.densidade_populacional);
  ...
  // }
    ...
    // if (atributo_escolhido > 7 || atributo_escolhido < 1) {
    //   printf("Atributo escolhido nao é valido. Verifique a variavel "
    //          "'atributo_escolhido'.\n");
    // }
}
...
int main() {
  ...
  //  Cadastro(&carta1, 1);Cadastro(&carta2, 2)
  // e inserir manualmente
  // (estou com preguiça de cadastrar cartas toda hora)
  struct Carta carta1 = {
  ...
  int atributo_escolhido = DENSIDADE_POPULACIONAL;
